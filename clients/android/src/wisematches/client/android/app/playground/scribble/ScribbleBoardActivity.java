...
public class ScribbleBoardActivity extends WiseMatchesActivity {
	public void onCreate(Bundle savedInstanceState) {
		...
		ProgressTask<Long, Void, ScribbleBoard> a = new ProgressTask<Long, Void, ScribbleBoard>("Loading board #" + boardId, this) {
			@Override
			protected ScribbleBoard doInBackground(Long... voids) {
				try {
					...
					final ServerResponse r = new ServerResponse(true, board);
					//getWMServer().post("/playground/scribble/board/load.ajax?b=" + boardId);
					if (r.isSuccess()) {
						...
						return new ScribbleBoard(id, settings, players, moves, new ScoreEngine(bonuses, 30));
					} else {
			...
			@Override
			protected void onPostExecute(ScribbleBoard board) {
	...
	private void showBoardInfo(ScribbleBoard board) {
		...
		viewById.setTileSelectionListener(new TileSelectionListener() {
			@Override
			...
			public void onWordSelected(ScribbleWord word) {
				if (word == null) {
					...
					viewById2.setText(word.getRow() + " " + word.getColumn() + " " + word.getDirection() + " " + word.getText());
				}
			}
		});
