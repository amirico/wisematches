...
public class MySQLNotificationManager implements NotificationManager {
	...
	public Date getNotificationDate(Personality personality, String code) {
		...
		final Query sqlQuery = session.createSQLQuery("select `" + code + "` from player_notification where pid=:pid");
	...
	public NotificationSettings getNotificationSettings(Personality personality) {
		...
		final Query sqlQuery = session.createSQLQuery("select " + b + " from player_notification where pid=:pid");
		sqlQuery.setLong("pid", personality.getId());
	...
	public void setNotificationSettings(Personality personality, NotificationSettings settings) {
		...
		final String query = "INSERT INTO player_notification (`pid`, " + b + ") VALUES (:pid, " + b2 + ") ON DUPLICATE KEY UPDATE " + b3;
		...
		for (int i = 0, changesSize = changes.size(); i < changesSize; i++) {
			...
			sqlQuery.setTimestamp("change" + i, settings.isEnabled(change) ? null : DISABLED);
		}
	...
	private void updateNotificationDate(Notification notification, Date date) {
		...
		final Session session = sessionFactory.getCurrentSession();
		final String query = "INSERT INTO player_notification (`pid`, `" + notification.getCode() + "`) VALUES (:pid, :time) " +
				"ON DUPLICATE KEY UPDATE `" + notification.getCode() + "`=:time";
		final Query sqlQuery = session.createSQLQuery(query);
		sqlQuery.setLong("pid", notification.getRecipient().getId());
