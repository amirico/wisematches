...
public class ScribbleGameController extends WisematchesController {
    ...
    public String createGamePage(@ModelAttribute("create") CreateScribbleForm form, Model model, Locale locale) {
        ...
        return "/content/playground/scribble/create2";
    }
    ...
    @RequestMapping("join")
    @Transactional(propagation = Propagation.SUPPORTS)
    public String joinGamePage(Model model, Locale locale) throws RestrictionException {
        final Player principal = getPrincipal();
    ...
    public String joinGameAction(@RequestParam("p") String id, Model model, Locale locale) throws BoardManagementException, RestrictionException {
        ...
        return joinGamePage(model, locale);
    }

    @RequestMapping("active")
    @Transactional(propagation = Propagation.SUPPORTS)
    public String activeGamesPage(Model model, Locale locale) {
        final Player principal = getPrincipal();
        if (log.isDebugEnabled()) {
    ...
    public ServiceResponse cancelProposalAjax(@RequestParam("p") String proposal, Model model, Locale locale) {
    ...
    private Collection<Player> getChallengedPlayers(CreateScribbleForm form) {
        for (String opponent : form.getOpponents()) {

        }
        return null;
    }

    private Collection<RobotPlayer> getRobotPlayers(CreateScribbleForm form) {
        return null;
    }

    private GameRestriction getGameRestriction(CreateScribbleForm form) {
        return null; // not supported at this moment
    }
