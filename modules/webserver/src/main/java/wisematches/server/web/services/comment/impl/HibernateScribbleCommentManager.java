...
public class HibernateScribbleCommentManager extends HibernateDaoSupport implements ScribbleCommentManager {
	...
	public void addComment(Player player, ScribbleBoard board, String text) {
		getHibernateTemplate().save(new HibernateScribbleComment(player, board, text));
	}
	...
	public long[] getBoardComments(ScribbleBoard board) {
		List l = getHibernateTemplate().find("select id from wisematches.server.web.services.comment.impl.HibernateScribbleComment where board=? order by creationDate, id desc", board.getBoardId());
		if (l.size() == 0) {
			return EMPTY_ARRAY;
		}
