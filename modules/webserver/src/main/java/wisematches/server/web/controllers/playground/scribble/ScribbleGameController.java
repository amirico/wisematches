...
public class ScribbleGameController extends WisematchesController {
	...
	public String showActiveGames(@RequestParam(value = "p", required = false) Long pid, Model model) throws UnknownEntityException {
		...
		if (principal == getPrincipal()) {
			final Collection<GameProposal<ScribbleSettings>> proposals =
					proposalManager.searchEntities(principal, ProposalRightholder.INITIATED, null, null, null);
	...
	public String showWaitingGames(Model model) throws RestrictionException {
		...
		final List<GameProposal<ScribbleSettings>> proposals = proposalManager.searchEntities(principal, ProposalRightholder.ANY, null, null, null);
		if (log.isDebugEnabled()) {
			log.debug("Found " + proposals.size() + " proposals for personality: " + principal);
		}
	...
	public String joinGameAction(@RequestParam("p") long id, Model model) throws BoardManagementException, RestrictionException {
		...
		try {
			...
			proposal = proposalManager.accept(id, principal);
			if (proposal == null) {
			...
			CriterionViolation criterion = e.getViolatedCriterion();
			if (criterion != null) {
				model.addAttribute("joinError", "game.error.restriction." + criterion.getCode() + ".description");
				model.addAttribute("joinErrorArgs", new Object[]{criterion.getReceived(), criterion.getExpected()});
			}
		}
	...
	public String declineProposalAction(@RequestParam("p") long id, Model model) throws BoardManagementException, RestrictionException {
		...
		try {
			final Player principal = getPrincipal();
			final GameProposal<ScribbleSettings> proposal = proposalManager.reject(id, principal);
			if (proposal == null) {
				model.addAttribute("joinError", "game.error.restriction.ready.description");
			}
		} catch (ViolatedCriteriaException e) {
...
//		int finishedGames = boardManager.getTotalCount(getPersonality(), GameState.FINISHED);
	...
	private int getActiveGamesCount(Player principal) {
		return boardManager.getTotalCount(principal, GameState.ACTIVE) + proposalManager.getTotalCount(principal, ProposalRightholder.ACTIVE);
	}
