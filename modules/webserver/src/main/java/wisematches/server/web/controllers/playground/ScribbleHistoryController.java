...
public class ScribbleHistoryController extends WisematchesController {
	...
	public String showHistoryGames(Model model, Locale locale) {
	...
	public Map<String, Object> loadHistoryGames(@RequestBody Map<String, Object> request, Locale locale) {
		final Player principal = getPrincipal();
		...
		final List<ScribbleGameHistory> games = historyManager.getFinishedGames(principal,
				Range.limit((Integer) request.get("iDisplayStart"), (Integer) request.get("iDisplayLength")), orders);
