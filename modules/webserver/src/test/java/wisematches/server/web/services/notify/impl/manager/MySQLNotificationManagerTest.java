...
public class MySQLNotificationManagerTest {
    ...
    public void testNotificationSettings() {
        ...
        for (NotificationDescriptor d : descriptions) {
            assertTrue(notificationManager.getNotificationScope(person, d.getCode()));
        }
        ...
        for (NotificationDescriptor d : descriptions) {
            assertFalse(notificationManager.getNotificationScope(person, d.getCode()));
        }
        settings = notificationManager.getNotificationSettings(person);
        for (String name : settings.getNotificationNames()) {
    ...
    @Test
    public void testLastNotificationDate() throws DeliveryException {
        ...
        expect(internalPublisher.publishNotification(isA(NotificationDeliveryServiceImpl.NotificationOld.class))).andReturn(true);
        replay(internalPublisher);

        final NotificationPublisherOld externalPublisher = createStrictMock(NotificationPublisherOld.class);
        expect(externalPublisher.publishNotification(isA(NotificationDeliveryServiceImpl.NotificationOld.class))).andReturn(true);
