...
public class ScribbleBoardDaoTest extends AbstractTransactionalJUnit4SpringContextTests {
	...
	public void test_getWaitingGames() {
		...
		final wisematches.server.games.dictionary.Dictionary dictionary = createStrictMock(wisematches.server.games.dictionary.Dictionary.class);
	...
	public void test_saveLoadScribbleBoard() throws GameMoveException {
		...
		final wisematches.server.games.dictionary.Dictionary dictionary = createStrictMock(wisematches.server.games.dictionary.Dictionary.class);
		expect(dictionary.getLocale()).andReturn(LOCALE);
		expect(dictionary.getWord("aaaa")).andReturn(new wisematches.server.games.dictionary.Word("aaaa", LOCALE));
		replay(dictionary);
