...
class AnalizingTree {
    ...
    private List<AnalizingTreeItem> treeItemList;
    private Deque<List<AnalizingTreeItem>> levels = new LinkedList<List<AnalizingTreeItem>>();

    AnalizingTree(TilesPlacement tilesPlacement, List<WorkTile> tiles) {
        ...
        treeItemList = new ArrayList<AnalizingTreeItem>();
    }

    public boolean offerNextChar(char ch) {
        List<AnalizingTreeItem> newLevel = new ArrayList<AnalizingTreeItem>();
        if (currentLevel == 0) {
            for (WorkTile tile : tiles) {
                if (tile.getTile().getLetter() == ch) {
                    newLevel.add(new AnalizingTreeItem(null, tile, null));
                } else if (tile.getTile().isWildcard() && tile.getPosition() == null) {
                    newLevel.add(new AnalizingTreeItem(null, tile, tile.getTile().redefine(ch), null));
                }
            }
        } else {
            for (WorkTile tile : tiles) {
                ...
                for (AnalizingTreeItem item : treeItemList) {
                    ...
                    if (item.isPlaced()) { //Item is already placed
                        if (tilePosition != null) { //Tile has a position
                            if (item.direction == null) { //But word doesn't have direction yet
                                if (item.row == tilePosition.row - 1 && item.column == tilePosition.column) { // and tile is placed below item
                                    ...
                                    newLevel.add(new AnalizingTreeItem(item, tile, originalTile, Direction.VERTICAL));
                                    ...
                                    newLevel.add(new AnalizingTreeItem(item, tile, originalTile, Direction.HORIZONTAL));
                                }
                                ...
                                newLevel.add(new AnalizingTreeItem(item, tile, originalTile, item.direction));
                            }
                        ...
                        if (tilePosition != null) { //Tile has position.
                            ...
                            newLevel.add(new AnalizingTreeItem(item, tile, originalTile, null));
                        }
                    }
                }
            }
    ...
    static class AnalizingTreeItem {
        private final int level;
        private final Tile rightTile;
        private final WorkTile workTile;
        private final AnalizingTreeItem parent;

        private int row = -1;
        private int column = -1;
        private final Direction direction;

        AnalizingTreeItem(AnalizingTreeItem parent, WorkTile workTile, Direction direction) {
            this(parent, workTile, workTile.getTile(), direction);
        }

        AnalizingTreeItem(AnalizingTreeItem parent, WorkTile workTile, Tile rightTile, Direction direction) {
