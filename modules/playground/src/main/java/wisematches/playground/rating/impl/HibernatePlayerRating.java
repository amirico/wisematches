...
@NamedQueries({
		@NamedQuery(name = "player.rating",
				query = "SELECT r.rating " +
						"FROM wisematches.playground.rating.impl.HibernatePlayerRating r where r.playerId=?",
				hints = {@QueryHint(name = "org.hibernate.cacheable", value = "true")}
		),
		@NamedQuery(name = "player.position",
				query = "SELECT count( a.id ), a.id " +
						"FROM wisematches.playground.rating.impl.HibernatePlayerRating a, wisematches.playground.rating.impl.HibernatePlayerRating b " +
