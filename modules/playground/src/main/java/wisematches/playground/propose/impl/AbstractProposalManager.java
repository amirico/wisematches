...
public abstract class AbstractProposalManager<S extends GameSettings> implements GameProposalManager<S>, InitializingBean {
    ...
    public GameProposal<S> initiate(S settings, Player initiator, int opponentsCount, PlayerCriterion... criteria) {
        return registerProposal(new DefaultGameProposal<S>(proposalIds.incrementAndGet(), settings, initiator, new Personality[opponentsCount], criteria));
    }
    ...
    public GameProposal<S> terminate(long proposalId) {
        ...
        try {
            ...
            final List<Personality> players = new ArrayList<Personality>(proposal.getPlayers());
    ...
    @Override
    public int getTotalCount(Personality person, ProposalRelation context) {
        return getFilteredCount(person, context, null);
    }

    @Override
    public int getFilteredCount(Personality person, ProposalRelation context, SearchFilter filter) {
        lock.lock();
        try {
            int res = 0;
    ...
    @Override
    public List<GameProposal<S>> searchEntities(Personality person, ProposalRelation context, SearchFilter filter, Orders orders, Range range) {
        lock.lock();
        try {
            if (context == ProposalRelation.AVAILABLE) {
