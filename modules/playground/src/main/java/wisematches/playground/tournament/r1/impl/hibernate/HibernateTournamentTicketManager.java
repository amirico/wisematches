...
import wisematches.playground.tournament.r1.TournamentTicketBatch;
import wisematches.playground.tournament.r1.impl.AbstractTournamentTicketManager;
	...
	public TournamentTicketBatch getTournamentTicketBatch(Language language) {
		...
		try {
			...
			final Query q = session.createQuery("" +
					...
					"from wisematches.playground.tournament.r1.impl.hibernate.HibernateTournamentTicket t " +
		...
		transactionTemplate.execute(new TransactionCallbackWithoutResult() {
			...
			protected void doInTransactionWithoutResult(TransactionStatus status) {
				...
				try {
					...
					final Session session = sessionFactory.getCurrentSession();
					final Query query = session.createQuery("" +
							"from wisematches.playground.tournament.r1.impl.hibernate.HibernateTournamentPoster p " +
