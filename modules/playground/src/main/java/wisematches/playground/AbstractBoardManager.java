...
public abstract class AbstractBoardManager<S extends GameSettings, B extends AbstractGameBoard<S, ?>> implements BoardManager<S, B> {
	...
	@Override
	public <Ctx extends GameState> int getTotalCount(Personality person, Ctx context) {
		return getFilteredCount(person, context, null);
	}

	@Override
	public <Ctx extends GameState, Fl extends SearchFilter> int getFilteredCount(Personality person, Ctx context, Fl filter) {
		return loadPlayerBoardsCount(person, context, filter);
	}

	@Override
	public <Ctx extends GameState, Fl extends SearchFilter> List<B> searchEntities(Personality person, Ctx context, Fl filter, Orders orders, Range range) {
		if (log.isDebugEnabled()) {
