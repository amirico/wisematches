...
public class HibernateMessageManager extends HibernateDaoSupport implements MessageManager {
    ...
    public void sendNotification(Personality recipient, String template) {
        sendNotification(recipient, template, false);
    }
    ...
    @Override
    @Transactional(propagation = Propagation.MANDATORY)
    public void sendNotification(Personality recipient, String template, boolean quite) {
        if (recipient == null) {
            throw new NullPointerException("Recipient can't be null");
        }
        publishMessage(new HibernateMessage(recipient, template), quite);
    }
