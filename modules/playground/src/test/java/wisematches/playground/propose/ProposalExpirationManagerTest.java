...
public class ProposalExpirationManagerTest {
	...
	public void testTerminatorInitialization() throws Exception {
		...
		expect(proposal2.getCreationDate()).andReturn(new Date(time - 6 * MILLIS_IN_DAY + 100));
		replay(proposal2);
		...
		expect(proposal3.getCreationDate()).andReturn(new Date(time - 7 * MILLIS_IN_DAY + 100)); // expired
		replay(proposal3);
