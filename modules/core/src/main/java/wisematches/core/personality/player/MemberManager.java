 ...
 */
public final class MemberManager extends AbstractPlayerManager implements PlayerManager {
	private AccountManager accountManager;
	private MembershipManager membershipManager;

	private final Map<Long, Member> playerMap = new WeakHashMap<>();
	...
	@Override
	public Member getPlayer(Long pid) {
		Member player = playerMap.get(pid);
		if (player == null) {
	...
	private Member createMemberPlayer(Account account) {
		final MembershipCard card = membershipManager.getPlayerMembership(account);
		return new Member(account, card);
	}
	...
	private final class TheMemberPlayerListener implements AccountListener, MembershipListener {
		...
		public void accountRemove(Account account) {
			final Member remove = playerMap.remove(account.getId());
			if (remove != null) {
				firePlayerUnregistered(remove);
			} else {
				firePlayerUnregistered(createMemberPlayer(account));
			}
		}

		@Override
		public void accountUpdated(Account oldAccount, Account newAccount) {
			final Member memberPlayer = playerMap.get(oldAccount.getId());
		...
		public void membershipCardUpdated(Account account, MembershipCard oldCard, MembershipCard newCard) {
			final Member memberPlayer = playerMap.get(account.getId());
			if (memberPlayer != null) {
