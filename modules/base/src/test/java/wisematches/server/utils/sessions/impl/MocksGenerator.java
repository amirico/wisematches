...
class MocksGenerator {
	...
	void generateInterface(Class... inherits) {
		...
		name = "wisematches.server.utils.sessions.impl.MockInterface" + (interfaceIndex++);
	...
	void generateClass(boolean isAbstract, Class superClass, Class... inherits) {
		...
		name = "wisematches.server.utils.sessions.impl.MockClass" + (interfaceIndex++);
		final String asmName = name.replaceAll("\\.", "/");
...
*/
	...
	public void addImplementationAnnotation() {
		...
		annotationVisitor.visit("value", "wisematches.server.utils.sessions.impl.MockClass" + (interfaceIndex));
