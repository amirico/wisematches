...
public class PlayerStatisticManagerImplTest {
	...
	private PlayerStatisticManagerImpl playerStatisticManagerImpl;
	...
	public void init() {
		...
		playerStatisticManagerImpl = new PlayerStatisticManagerImpl();
		playerStatisticManagerImpl.setRatingManager(playerRatingManager);
		playerStatisticManagerImpl.setPlayerStatisticDao(playerStatisticDao);
		playerStatisticManagerImpl.setRoomsManager(roomsManager);
	...
	public void removeUser() throws UnknownAccountException {
		playerStatisticManagerImpl.setRoomsManager(null);
	}
	...
	public void testAccountManager() {
		...
		playerStatisticManagerImpl.setAccountManager(m);
	...
	public void test_lockUnlock() throws InterruptedException {
		...
		playerStatisticManagerImpl.lock(p1_1);
		final Future<PlayerStatistic> future = executorService.submit(new Callable<PlayerStatistic>() {
			public PlayerStatistic call() throws Exception {
				return playerStatisticManagerImpl.getPlayerStatistic(p1_2);
			}
		});
		final Future<PlayerStatistic> future2 = executorService.submit(new Callable<PlayerStatistic>() {
			public PlayerStatistic call() throws Exception {
				return playerStatisticManagerImpl.getPlayerStatistic(p2);
			}
		});
		Thread.sleep(300);
		...
		final PlayerStatistic statistic = playerStatisticManagerImpl.getPlayerStatistic(p1_2);
		assertNotNull(statistic);
		playerStatisticManagerImpl.unlock(p1_1);
	...
	public void test_getPreviousMoveTime() {
		...
		assertEquals(time - 1000, playerStatisticManagerImpl.previousMoveTime(gb).getTime());
		// returns last move time
		assertEquals(time - 15000, playerStatisticManagerImpl.previousMoveTime(gb).getTime());
	...
	public void test_updateTurnsStatistic() {
		...
		playerStatisticManagerImpl.processPlayerMoved(gb, new GameMove(move1, 10, 1, new Date(moveTime)));
		...
		playerStatisticManagerImpl.processPlayerMoved(gb, new GameMove(move1, 10, 1, new Date(moveTime + 3000)));
		...
		playerStatisticManagerImpl.processPlayerMoved(gb, new GameMove(move1, 10, 1, new Date(moveTime + 8000)));
	...
	public void test_updateRatingInfo() {
		...
		final HibernateRatingsStatistic ri = statistic.getRatingsStatistic();

		{
			...
			playerStatisticManagerImpl.updateRatingInfo(board, statistic, ri, Arrays.asList(change1, change2, change3));
	...
	@Test
	public void test_updateWordStatistic() {
		final HibernatePlayerStatistic statistic = new HibernatePlayerStatistic(Personality.person(14L));
		...
		playerStatisticManagerImpl.updateWordStatistic(board, move1, statistic);
		assertEquals(2, ws.getWordsCount());
		verify(board);
		throw new UnsupportedOperationException("Code commented");
	}
