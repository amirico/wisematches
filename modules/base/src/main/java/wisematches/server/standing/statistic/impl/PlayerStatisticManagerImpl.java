...
public class PlayerStatisticManagerImpl implements PlayerStatisticManager {
		...
		for (P hand : hands) {
			...
			try {
				...
				if (ratedGame) { // If game is not rated just ignore it
					...
					updateRatingsInfo(board, hand, (Collection<GamePlayerHand>) hands, ratingChanges, statistic);
				}
				updatePlayerStatistic(personality, statistic);
			} finally {
				unlock(personality);
	...
	protected void updateRatingsInfo(GameBoard board, GamePlayerHand hand, Collection<GamePlayerHand> hands,
									 Collection<RatingChange> changes, PlayerStatistic statistic) {
		updateRatingInfo(board, hand, hands, changes, statistic, statistic.getAllGamesStatisticRating());
/*
...
*/
	...
	protected void updateRatingInfo(GameBoard board, GamePlayerHand hand, Collection<GamePlayerHand> hands,
									Collection<RatingChange> changes, PlayerStatistic statistic, PlayerStatisticRating ri) {
		...
		final short averageOpponentsRating = (short) (opponentsRatings / (hands.size() - 1));
	...
	private void lock(Personality personality) {
		ReentrantLock lock;
		lockLock.lock();
		try {
	...
	private void unlock(Personality personality) {
		lockLock.lock();
		try {
	...
	private Date previousMoveTime(GameBoard board) {
		@SuppressWarnings("unchecked")
		final List<GameMove> list = board.getGameMoves();
		if (list.size() <= 1) {
	...
	private int average(final int previousAverage, final int newValue, final int newCount) {
		return (previousAverage * (newCount - 1) + newValue) / newCount;
	}

	private boolean isPlayerIgnored(final GamePlayerHand hand) {
		return isPlayerIgnored(hand.getPlayerId());
	}

	private boolean isPlayerIgnored(final long playerId) {
		return ComputerPlayer.getComputerPlayer(playerId) != null;
	}

	private void updatePlayerStatistic(Personality personality, PlayerStatistic statistic) {
