...
import wisematches.server.testimonial.lock.*;
		...
		final Date unlockDate = template.execute(new HibernateCallback<Date>() {
			public Date doInHibernate(Session session) throws HibernateException, SQLException {
				final Query query = session.createQuery("" +
						...
						"from wisematches.server.testimonial.lock.impl.HibernateLockAccountInfo lock " +
						"where lock.playerId = ?");
				...
				template.execute(new HibernateCallback<Void>() {
					public Void doInHibernate(Session session) throws HibernateException, SQLException {
						final Query q = session.createQuery(
								"delete from wisematches.server.testimonial.lock.impl.HibernateLockAccountInfo lock where lock.playerId = ?");
