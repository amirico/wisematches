...
wm.account = new function() {
    ...
    this.loadTermsPage = function(pageName) {
        Ext.get(currentTermsPage + '-page-link').removeClass('active');
        currentTermsPage = pageName;
        Ext.get(pageName + '-page-link').addClass('active');

        Ext.Ajax.request(
        {
            url: termsPages[pageName],
            success: function(response, options) {
                var html = response.responseText;
                var el = Ext.get('terms_panel');
                el.setHeight(Ext.get('form').getHeight() - Ext.get('terms_tabs').getHeight());
                el.update(html);
            }
        });
    };
